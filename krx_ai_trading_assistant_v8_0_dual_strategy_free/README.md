...
`V9_10YEAR_TRAINING_GUIDE.md`를 참고하세요. Windows에서는 `train_10year_v9.bat`를 실행하면 한국/미국 데이터 수집 → feature 생성 → 4개 모델 학습을 순서대로 수행합니다. 학습 모델이 존재하면 하루 +1% 및 1~2주 전략에 10년 패턴 점수가 자동으로 혼합되고, 모델이 없으면 기존 실시간 모델로 안전하게 fallback합니다.
