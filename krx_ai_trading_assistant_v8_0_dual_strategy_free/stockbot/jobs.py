...
EASY_FEATURES = {
    "liquidity": "거래가 활발함", "turnover_acceleration": "거래대금이 빠르게 늘어남",
    "momentum_1d": "오늘 주가 힘이 좋음", "momentum_5d": "최근 5일 흐름이 좋음",
    "momentum_10d": "최근 10일 흐름이 좋음", "trend_quality": "상승 추세가 비교적 안정적임",
    "foreign_flow": "외국인 수급이 양호함", "institution_flow": "기관 수급이 양호함",
    "news_catalyst": "최근 긍정 뉴스 재료가 있음", "broker_report_signal": "증권사 리포트 분위기가 양호함",
    "event_positive_strength": "긍정 공시·이벤트가 있음", "event_risk_inverse": "큰 악재 위험이 상대적으로 낮음",
    "overheating_inverse": "단기 과열이 심하지 않음", "volatility_quality": "가격 흔들림이 과도하지 않음",
    "atr_pct_quality": "당일 매매에 필요한 움직임이 충분함", "near_20d_high": "최근 고점 부근에서 강하게 버팀",
    "range_position_20d": "최근 가격대의 상단에 위치", "close_location_value": "최근 종가가 강하게 마감",
    "volume_pressure": "매수 거래량 힘이 좋음", "macro_global_risk_on": "글로벌 시장 분위기가 우호적임",
    "market_live_risk_on": "현재 전체 시장 분위기가 우호적임", "market_semis_strength": "반도체 시장 흐름이 강함",
}

def easy_reasons(features: dict[str, Any], limit: int = 3) -> list[str]:
    rows = []
    for k, label in EASY_FEATURES.items():
        try: v = float(features.get(k, 0.5))
        except Exception: v = 0.5
        rows.append((v, label))
    return [label for v, label in sorted(rows, reverse=True) if v >= 0.56][:limit]
...
class TradingAssistant:
    ...
    def format_premarket(self, plans: list[TradePlan], market_summary: str) -> str:
        lines = [f"📌 {self.today()} 장전 추천 요약", market_summary or "시장 요약 없음", ""]
        if not plans:
            lines += [
        ...
        for i, p in enumerate(plans, 1):
            lines += [
                f"{i}. {p.name} ({p.code}) | 추천점수 {p.score:.1f}/100",
                "✅ 왜 뽑혔나: " + (" · ".join(easy_reasons(p.features, 3)) or (p.rationale or "여러 지표가 고르게 양호함")),
                ...
                f"권장 최대 비중 {p.weight_pct:.1f}% | 약 {float(self.settings.get('risk.day_trade_capital_krw', 0)) * p.weight_pct / 100:,.0f}원 | 약 {int((float(self.settings.get('risk.day_trade_capital_krw', 0)) * p.weight_pct / 100) // max(p.entry_high_1, 1))}주 | 모델 확신 {p.confidence:.0f}/100",
                (f"AI 보조판단: 상승확률 {p.up_probability:.1f}% | 예상수익 {p.expected_return_pct:+.2f}% | 장중 기대 최대상승/하락 {p.expected_mfe_pct:+.2f}/{p.expected_mae_pct:+.2f}% | 예상범위 {p.lower_return_pct:+.2f}~{p.upper_return_pct:+.2f}%" if p.model_active else "AI 보조판단: 아직 학습표본이 부족해 기본 점수 중심"),
                f"세부 메모: {p.rationale or '수치 모델 우선 선정'}",
    ...
    def format_close(self, selections: list[dict[str, Any]], outcomes: list[dict[str, Any]], learning: dict[str, Any], broker_report_count: int = 0, model_feedback: dict[str, Any] | None = None, shadow_count: int = 0) -> str:
        ...
        for s in selections:
            ...
            lines.append(
                f"• {s['name']}: 시가→종가 {float(s.get('open_to_close_pct') or 0):+.2f}% "
                f"| 장중 최고기회 {float(s.get('mfe_pct') or 0):+.2f}% | 장중 최대하락 {float(s.get('mae_pct') or 0):+.2f}%"
            )
            if o.get("entered"):
                lines.append(
        ...
        if model_feedback.get("predictions"):
            lines += [
                ...
                f"종목 순위 적중도 {float(model_feedback.get('cross_sectional_rank_ic', 0)):+.3f} | 실제 수익이 예상범위 안에 든 비율 {100*float(model_feedback.get('interval_coverage', 0)):.1f}%",
            ]
        if learning.get("updated"):
            ...
            lines.append(f"오늘 학습 반영: 장전 후보 {learning.get('samples_today', 0)}개 결과를 다음 추천에 반영")
