...
class USMarketAssistant:
    ...
    def discover(self) -> tuple[list[Candidate], dict[str,pd.DataFrame], str, dict[str,str]]:
        ...
        for ex in exchanges:
            try:
                ...
                log.warning("US turnover rank stopped: KIS network unavailable at %s: %s", ex, exc)
                return [], {}, "KIS network temporarily unavailable; previous dashboard state preserved", {}
        ...
        for sym, seed in list(rows.items())[: max(60,pool_size*4)]:
            ...
            try:
                ...
                log.warning("US discovery stopped: KIS network unavailable at %s/%s: %s", ex, sym, exc)
                return [], dmap, "KIS network temporarily unavailable; previous dashboard state preserved", exmap
            except Exception as exc:
    ...
    def intraday(self, force_summary:bool=False, scan_replacement: bool=False)->list[dict[str,Any]]:
        ...
        for r in recs:
            ...
            try:
                ...
                log.warning("US intraday stopped: KIS network unavailable at %s: %s", sym, exc)
                break
            except Exception as exc:
    ...
    def close(self)->dict[str,Any]:
        ...
        for r in shadows:
            sym=str(r["code"]); ex=str(exmap.get(sym,"NAS"))
            try:
                bars=self.kis.overseas_intraday_chart(sym,ex,5,include_previous=False)
            except KISNetworkError as exc:
                log.warning("US close stopped: KIS network unavailable at %s: %s", sym, exc)
                break
            except Exception as exc:
                log.warning("US close bars failed %s: %s",sym,exc); continue
